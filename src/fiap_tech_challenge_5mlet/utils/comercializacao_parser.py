...
def parse_comercializacao(html: str) -> list[dict]:
    """
    Parses the HTML content from Embrapa's 'Comercialização' page
    and returns a list of products and their related subproducts with
    quantities.
    ...
    Returns:
        list[dict]: Structured data of products and subproducts.

    Raises:
        HTTPException: If the data table is not found or contains only empty
        values.
    """
    ...
    if not table:
        raise HTTPException(status_code=404,
                            detail=("Comercialização table not found."))
    ...
    for row in rows:
        ...
        # Identify subproducts linked to the last main product
    ...
    if all_empty:
        raise HTTPException(
            status_code=404,
            detail="No comercialização data available for the specified year."
        )

    # Look for the footer with total values
